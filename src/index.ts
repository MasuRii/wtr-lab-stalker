(function () {
  ...
  function getMobileSearchButton(): HTMLButtonElement | null {
    ...
    const button = document.querySelector<HTMLButtonElement>(
      'nav button:has(use[href="#search"]), nav button:has(use[xlink\\:href="#search"])',
    );
  ...
  function findBrandIcon(navbar: Element): SVGElement | null {
    ...
    const logoPath = navbar.querySelector<SVGPathElement>('a svg path');
    if (logoPath) {
    ...
    return brand?.querySelector<SVGElement>('svg') ?? null;
  }

  function findSearchContainer(input: HTMLInputElement): HTMLElement | null {
  ...
  function mount(): boolean {
    ...
    const input = navbar.querySelector<HTMLInputElement>('input[type="search"], input[aria-label="Search"], input');
    if (!brandIcon || !input) return false;
